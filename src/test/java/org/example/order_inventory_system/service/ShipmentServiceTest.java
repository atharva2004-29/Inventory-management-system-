...
//        assertEquals("123 MG Road, Mumbai", result.getDeliveryAddress());
...
//        when(shipmentRepository.findByShipmentStatus("CREATED"))
...
class ShipmentServiceTest {
    ...
    void findByStatus_ShouldReturnMatchingShipments() {
        ...
        assertEquals(1, result.size());
        assertEquals("CREATED", result.get(0).getShipmentStatus());
        verify(shipmentRepository, times(1)).findByShipmentStatus("CREATED");
    }
    ...
    void findByCustomerId_ShouldReturnShipmentsForCustomer() {
        ...
        assertEquals(1, result.size());
        assertEquals("John Doe", result.get(0).getCustomer().getFullName());
        verify(shipmentRepository, times(1)).findByCustomer_CustomerId(1);
    }
}